...
export default function Appointments() {
    ...
    return (
        <div className="w-full">
            ...
            {/* Tabs - Mobile par scroll honge */}
            <div className="flex gap-2 mb-6 border-b border-gray-200 overflow-x-auto pb-px">
                {TABS.map((t) => (
                    <button
                        key={t}
                        onClick={() => setTab(t)}
                        className={`px-4 py-2 text-sm font-medium capitalize transition-colors border-b-2 whitespace-nowrap ${tab === t ? "border-brand-700 text-brand-700" : "border-transparent text-gray-500 hover:text-gray-700"
                            }`}
                    >
                        {t}
                    </button>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 divide-y divide-gray-100">
                {appointments?.map((a) => (
                    <div key={a._id} className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        ...
                        {/* Action Buttons - Mobile par right side mein aayenge */}
